...
class Calibrate_Qt(object):
    ...
    def __init__(self, raw_data, geofile=None, vmin=-1000, vmax=5000, **kwargs):
        ...
        '''
        assert raw_data.shape == (16, 512, 128) #Only one image should be passed
        ...
        self.info = QtGui.QLabel('Click on Quadrant to select; CTRL+arrow-keys to move')
    ...
    def _drawCircle(self):
        ...
        fit_helper.sigRegionChangeFinished.connect(lambda: self.sel3.widgets.set_val(fit_helper))
...
class CalibTab(widgets.VBox):
    ...
    def _move_quadrants(self, pos):
        ...
        self.parent._draw_rect({0:None, 1:2, 2:1, 3:4, 4:3}[self.parent.quad])
...
class Calibrate_Nb(object):
    ...
    def update_plot(self, val=(100, 1500), cmap='gist_earth' ):
        ...
        if not self.im is None:
            ...
            self.im = self.ax.imshow(self.data, vmin=val[0], vmax=val[1], cmap=self.cmap)
